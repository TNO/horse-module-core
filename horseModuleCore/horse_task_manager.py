...
class HorseTaskManager(HorseWorkerModule):
    def __init__(self, sender_id=None):
        super(HorseTaskManager, self).__init__('HorseTaskManager')
        
        if sender_id == None:
            self.sender_id = horseModuleCore.config.HORSE['msgbus']['sender_id']
        else:
            self.sender_id = sender_id
        
        self.hmc = HorseMessageClient(sender_id = self.sender_id);
